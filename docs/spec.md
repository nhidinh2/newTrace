# NewsTrace — project specification

The original design document for NewsTrace, kept for reference. It is the
brief the implementation was built against, so parts of it describe intent
rather than shipped behaviour; [`../README.md`](../README.md) and the other
files in `docs/` describe what actually exists.
...
| | |
| --- | --- |
| ![Stories](screenshots/stories.png) | ![Story](screenshots/story.png) |
| **Stories** — evolving stories with article and independent-source counts | **Story** — timeline, earliest available report, and near-duplicates marked as not independent |
| ![Search](screenshots/search.png) | ![Experiments](screenshots/experiments.png) |
| **Search** — every hit carries provenance and a "why this ranked here" line | **Experiments** — full versus compressed retrieval, from saved metrics |

The story shot is the duplication behaviour working on real data: nine
articles, eight of them Australian regional syndications of one wire story,
counted as **one** independent source rather than nine.
...
Pages are grouped: *Read* for the news, *Inspect* for the machinery.

1. **Stories** — clusters ordered by recency and coverage.
2. **Story** — timeline, source list, claims, evidence, and duplicate count.
3. **Search** — query, filters, results, and retrieval explanations.
4. **Topics** — read-only view; configuration remains YAML-based.
5. **Experiments** — quality-versus-memory and quality-versus-latency plots.
6. **Health** — last ingestion run, failures, counts, and model/index versions.
...
[`docs/experiments/live-30d/metrics.json`](experiments/live-30d/metrics.json)
